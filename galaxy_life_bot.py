...
class GalaxyBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        # 1) Initialize our database connection & schema
        self.pool = await asyncpg.create_pool(DATABASE_URL, min_size=1, max_size=5)
        await self._init_db()

        # 2) Clear out _all_ existing slash commands
        #    - global (guild=None) and
        #    - in our test guild (if provided)
        self.tree.clear_commands(guild=None)
        if TEST_GUILD:
            self.tree.clear_commands(guild=TEST_GUILD)

        # 3) Re-register _all_ @bot.tree.command decorators into the test guild
        if TEST_GUILD:
            self.tree.copy_global_to(guild=TEST_GUILD)
            await self.tree.sync(guild=TEST_GUILD)
            print(f"❇ Cleared & re-synced slash commands to test guild {TEST_GUILD.id}")
        else:
            # Optionally sync global for production
            await self.tree.sync()
            print("✅ Cleared & synced global slash commands")
...
async def get_members_with_colonies(alliance: str) -> List[Tuple[str,int,List[Tuple[int,int]]]]:
    ...
    ret: List[Tuple[str,int,List[Tuple[int,int]]]] = []
    for r in rows:
        coords = [tuple(map(int,s.split(','))) for s in r[2]] if r[2] else []
        ret.append((r[0], r[1], coords))
    return ret
...
def member_ac_factory(param_alliance: str):
    async def _ac(inter: discord.Interaction, current: str):
        ...
        return [app_commands.Choice(name=r[0], value=r[0])
                for r in rows if current.lower() in r[0].lower()][:25]
    return _ac
...
async def show(inter: discord.Interaction, alliance: str):
    ...
    if not members:
        ...
        for name,cnt,coords in members:
            ...
            embed.add_field(name=f"{name} ({cnt}/{MAX_COLONIES})", value=cs, inline=False)
