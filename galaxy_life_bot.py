...
========================================================
Persistent across every deploy **without volumes**. Data in Railway Postgres.
...
if not DATABASE_URL:
    raise RuntimeError("Set DATABASE_URL via Railway Postgres plugin")
    ...
    async def _init_db(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                -- colonies: PK on alliance,member,x,y; starbase is just a property
...
async def get_members_with_colonies(alliance: str) -> List[Tuple[str,int,List[Tuple[int,int,int]]]]:
    query = """
        SELECT m.member,
               COUNT(c.x) AS ncol,
               COALESCE(
                 array_agg(c.x||','||c.y||','||c.starbase ORDER BY c.x,c.y),
                 '{}'
               ) AS coords
        FROM members m
...
async def addcolony(
    ...
    if await colony_count(alliance, member) >= MAX_COLONIES and \
       not await bot.pool.fetchval(
         "SELECT 1 FROM colonies WHERE alliance=$1 AND member=$2 AND x=$3 AND y=$4",
         alliance, member, x, y
       ):
        return await inter.response.send_message("❌ Already have 11 distinct colonies.", ephemeral=True)

    # UPSERT: if same alliance/member/x/y exists, update starbase; else insert
    async with bot.pool.acquire() as conn:
        await conn.execute("""
    ...
    await inter.response.send_message(f"✅ Colony SB{sb} @ {x},{y} saved.", ephemeral=True)
