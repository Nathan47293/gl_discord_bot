...
class GalaxyBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        if TEST_GUILD:
            ...
            print("Global commands synced (up to 1h to propagate)")
...
async def addmember(inter: discord.Interaction, alliance: str, member: str):
    if not await alliance_exists(alliance): return await inter.response.send_message("Alliance not found.", ephemeral=True)
    if await member_exists(alliance, member):     return await inter.response.send_message("Member already exists.", ephemeral=True)
    async with bot.pool.acquire() as conn:
...
async def addcolony(inter: discord.Interaction, alliance: str, member: str, x: int, y: int):
    if not await member_exists(alliance, member): return await inter.response.send_message("Member not found.", ephemeral=True)
    if await colony_count(alliance, member)>=MAX_COLONIES: return await inter.response.send_message("Max colonies reached.", ephemeral=True)
    async with bot.pool.acquire() as conn:
...
async def show(inter: discord.Interaction, alliance: str):
    ...
    if not members: embed.description="No members recorded."
    else:
        for name,count,coords in members:
            cs=", ".join(f"{x},{y}" for x,y in coords) or "None"
            embed.add_field(name=f"{name} ({count}/{MAX_COLONIES})",value=cs,inline=False)
    await inter.response.send_message(embed=embed)

@bot.tree.command(description="List all alliances.")
async def list(inter: discord.Interaction):
    names=await all_alliances()
    if not names: return await inter.response.send_message("No alliances recorded.", ephemeral=True)
...
async def removecolony(inter: discord.Interaction, alliance: str, member: str, x: int, y: int):
    if not await member_exists(alliance, member): return await inter.response.send_message("Member not found.", ephemeral=True)
    res=await bot.pool.acquire().then(lambda c: c.execute(
        "DELETE FROM colonies WHERE alliance=$1 AND member=$2 AND x=$3 AND y=$4", alliance, member, x, y
    ))
    if res.endswith("0"): return await inter.response.send_message("No such colony.", ephemeral=True)
...
async def renamemember(inter: discord.Interaction, alliance: str, old: str, new: str):
    if not await member_exists(alliance, old): return await inter.response.send_message("Original not found.", ephemeral=True)
    if await member_exists(alliance, new): return await inter.response.send_message("New name taken.", ephemeral=True)
    async with bot.pool.acquire() as conn:
        await conn.execute("UPDATE members SET member=$1 WHERE alliance=$2 AND member=$3", new, alliance, old)
        await conn.execute("UPDATE colonies SET member=$1 WHERE alliance=$2 AND member=$3", new, alliance, old)
