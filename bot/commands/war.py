...
async def _show_war(inter: discord.Interaction):
    ...
    async with pool.acquire() as conn:
        ...
        E = await conn.fetchval(
            "SELECT COUNT(*) FROM members WHERE alliance=$1", war["enemy_alliance"]
        )
        wp_map = {1:100,2:200,3:300,4:400,5:600,6:1000,7:1500,8:2000,9:2500}
        main_enemy = await conn.fetch(
            "SELECT main_sb FROM members WHERE alliance=$1",
            war["enemy_alliance"]
        )
        col_enemy = await conn.fetch(
            "SELECT starbase FROM colonies WHERE alliance=$1",
            war["enemy_alliance"]
        )
...
class WarCog(commands.Cog):
    ...
    async def attack(
        ...
        if await get_current_war(self.bot.pool, gid):
            return await inter.response.send_message(
                "❌ A war is already in progress! Use `/war`.", ephemeral=True
            )
    ...
    async def endwar(
        ...
        await self.bot.pool.execute(
            "DELETE FROM wars WHERE guild_id=$1", str(inter.guild_id)
        )
